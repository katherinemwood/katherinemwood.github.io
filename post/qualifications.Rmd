...
Here, I'm going to demonstrate how to make a qualification test that auto-scores itself using the boto3 library for Python. It's a simple, 2-question colorblindness test (you can view it on the worker sandbox [here](https://workersandbox.mturk.com/mturk/requestqualification?qualificationId=3CFGE88WF7UDUETM7YP3RSRD73VS4F); sign-in required) that first asks people to self-report any known issues with their color vision, and then has them respond to an Ishihara plate. Here's what it looks like to a worker:

![a qualification test in the wild](/img/qualification_test.png)
...
For more detail on how to handle setting all of this up, or to see how all the pieces fit together, you can check out [this post](katherinemwood.github.io/post/mturk_dev_intro/) for an in-depth guide.
