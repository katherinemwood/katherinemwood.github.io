...
These data are somewhat unusual, in that there is only one solution for each set of statistics. By way of illustration, here's how many solutions are returned if we take condition 1 and pretend we have a 5-point scale instead of 3:

![](/img/corvids_inc_scale.png "A larger scale")

CORVIDS still returns every and all possible solutions, but now that the scale is less constrained there are more possibilities for the data.